 ...
 */
//产品的户型数量
function doorcount($id){
	$member = M('diy')->where('pro_id='.$id)->count();//户型名
	return  $member;
}
//规格
function size($id){
	$member = M('standard')->field('name')->find($id);//户型名
	return  $member['name'];
}
//户型名
function doormodel($id){
	$member = M('doormodel')->field('name')->find($id);//户型名
	return  $member['name'];
}
//业务员用户名
function sala_username($id){
	$member = M('Ucenter_member')->field('username')->find($id);//业务员
	return  $member['username'];
}
//预留手机
function reserve_phone($id){
	$member = M('ucenter_member')->field('reserve_phone')->find($id);//预留手机
	return  $member['reserve_phone'];
}
//代理商邮箱
function email($id){
	$member = M('ucenter_member')->field('email')->find($id);//预留手机
	return  $member['email'];
}
//单位名称
function company($id){
	$member = M('ucenter_member')->field('company')->find($id);//单位名称
	return  $member['company'];
}

//根据product_group表中的id查找产品品牌
function brand($id){
	$member = M('produce_group')->field('name')->find($id);//单位名称
	return  $member['name'];
}
//根据产品的brand_id查找品牌
function get_brand($id){
	//$product_group = M("produce_group");
	$sql = "select name from bizhi_produce_group where id = (select pid from bizhi_produce_group where id=".$id.")";
	$brand_name = M('produce_group')->query($sql);
	return $brand_name[0]["name"];
}
function get_sample_name($id){
	$member = M('produce_group')->field('name')->find($id);//单位名称
	return  $member['name'];
}
//查询产品名称
function get_product_name($id){
	$member = M('produce')->field('name')->find($id);
	return  $member['name'];
}
//查询产品编号
function get_product_number($id){
	$member = M('produce')->field('number')->find($id);
	return  $member['number'];
}
//查询产品规格
function get_product_size($id){
	$member = M('produce')->field('size')->find($id);
	return  $member['size'];
}
/*//代理价
function get_daili_price($produce_id,$pr){
...
function get_base_price($id){
	...
	$amap['uid'] = UID;
	$agent = M('Agent')->where($amap)->field('puid')->find();
...
//零售价
function get_price($produce_id,$pr){
...
//代理商
function agent_name($status) {
	switch ($status){
		case 1  : return    '代理商';     break;
		case 2  : return    '经销商';     break;
		case 3  : return    '设计师';       break;
